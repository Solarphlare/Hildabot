...
dpp::task<bool> super_streak_saver::give_item(const dpp::button_click_t& event) const {
    ...
    if (!streak_expiry) {
        co_await this->send_success_message(event, "Your streak will be extended by 1 week should it expire.");
        co_return true;
    }
    ...
    co_await this->send_success_message(event, std::format("Your streak will be extended by 1 week should it expire.\n### Effective streak expiry:\n<t:{}>", new_expiry));
    co_return true;
}
